# GAPPS-Mi439
These are GAPPs packages from NikGapps Specifically For Mi439 devices😉


# Credits
* NikGapps Team for providing us automated system and tools to flash Custom Gapps
* Google For providing Logos for them and obviously Gapps for us to use in our ROMs


# Details
* Here i Will post My Gapps Configs
* It is not restricted that they will only work on this if you wanted and liked my Configs you can use it.
* I am Not responsible for bricked devices, dead sd cards, or anything bad happening.
* It is listed under GPL-3.0 Licence


# Gapps included into package
* Google Dialer.                    📞
* Google Photos                     🖼️
* Google messages                   📥
* Google play store with services   🏪
* Google Search Velvit.           **G**
* Google calander 📆
* Google calculator ➗
* Google clock 🕐
* Google contacts 🧺
* Google assistant 🤖
* Pixel setup wizard 🧙‍♀️
* And i forgot maybe that's it
